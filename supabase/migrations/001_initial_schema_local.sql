-- ============================================================================
-- Performance App — Migración Inicial (PostgreSQL Local)
...
-- 1.1 Líneas de envasado
CREATE TABLE public.lines (
    id                    UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name                  TEXT NOT NULL UNIQUE,
    nominal_speed_bph     INTEGER NOT NULL DEFAULT 60000,
    bottle_volume_liters  NUMERIC(6,4) NOT NULL DEFAULT 0.6300,
    target_hl_per_hour    NUMERIC(8,2) NOT NULL DEFAULT 378.00,
    target_hl_per_day     NUMERIC(10,2) NOT NULL DEFAULT 9072.00,
...
CREATE TABLE public.profiles (
    ...
    password_hash TEXT, -- para auth local en dev; en prod se usa Supabase Auth
...
SELECT
    ...
    -- Volumen Real (HL) = (botellas × volumen_botella) / 100
    ROUND(
    ...
    -- GLY % = (botellas / velocidad_nominal) × 100
    CASE
...
VALUES
    ('admin@performance.local',      'Administrador',       'admin'),
    ('supervisor@performance.local', 'Supervisor Planta',   'supervisor'),
    ('operador1@performance.local',  'Operador Línea 1',    'operador'),
    ('operador3@performance.local',  'Operador Línea 3',    'operador')
ON CONFLICT (email) DO NOTHING;

-- Asignar líneas a los operadores
UPDATE public.profiles
SET line_id = (SELECT id FROM public.lines WHERE name = 'Línea 1')
WHERE email = 'operador1@performance.local';

UPDATE public.profiles
SET line_id = (SELECT id FROM public.lines WHERE name = 'Línea 3')
WHERE email = 'operador3@performance.local';
